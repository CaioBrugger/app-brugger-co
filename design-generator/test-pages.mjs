...
const pages = [
  { name: 'Agents',    url: 'http://localhost:3000/agents',    expectCards: true,  expectedCount: 20 },
  { name: 'Skills',    url: 'http://localhost:3000/skills',    expectCards: true,  expectedCount: 37 },
  { name: 'Workflows', url: 'http://localhost:3000/workflows', expectCards: true,  expectedCount: 11 },
  { name: 'Extractor', url: 'http://localhost:3000/extractor', expectCards: false, expectedCount: 0  },
];
...
async function testPage(context, info) {
  ...
  try {
    ...
    // Se a página ficou em branco, aguarda mais 5s (reload do Vite)
    const bodyText1 = await page.locator('body').innerText().catch(() => '');
    if (bodyText1.trim().length < 10) {
      console.log('  Pagina em branco, aguardando mais 5s (possivel reload do Vite)...');
      await page.waitForTimeout(5000);
    }
    ...
    // Extractor: checar input URL e botao
    let extractorInfo = null;
    if (info.name === 'Extractor') {
      const urlInputCount = await page.locator('input[type="url"], input[placeholder*="url" i], input[placeholder*="URL"], input[name*="url" i], input[type="text"]').count();
    ...
    let passed = false;
    if (info.expectCards) {
      passed = agentCardCount >= Math.floor(info.expectedCount * 0.8);
    } else {
...
(async () => {
  ...
  for (const r of results) {
    ...
    if (r.status === 'ERRO') {
      ...
      console.log(`Seletores com resultado: ${JSON.stringify(r.cardCounts)}`);
      if (r.extractorInfo) console.log(`Extractor info: ${JSON.stringify(r.extractorInfo)}`);
      if (r.errorTexts.length) console.log(`Erros visiveis: ${JSON.stringify(r.errorTexts)}`);
      if (r.consoleErrors.length) console.log(`Console errors: ${JSON.stringify(r.consoleErrors)}`);
      if (r.has404) console.log('AVISO: Parece 404');
      if (r.hasLoading) console.log('AVISO: Parece estar em loading');
      console.log(`Trecho body: ${r.bodySnippet}`);
